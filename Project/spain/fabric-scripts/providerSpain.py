...
class MyTCPHandler(socketserver.BaseRequestHandler):
    ...
    def handle(self):
        ...
        response = muterun_js('/fabric-scripts/invokeChaincode/querySpain.js')
        if response.exitcode == 0:
            print("Successfully queried chaincode")
        else:
            print("Unsuccessfully queried chaincode")
            self.request.sendall(b'Unsuccessfully queried chaincode')
        ...
        print("userOTC", user_otc)
        print("ledgerOTC", ledger_otc)
